...
def git_pieces_from_vcs(tag_prefix, root, verbose, run_command=run_command):
    ...
    if "-" in git_describe:
        ...
        if tag_is_glob_pattern:
            if not fnmatch.fnmatch(full_tag, tag_prefix):
                if verbose:
                    print(f"tag '{full_tag}' doesn't match glob pattern '{tag_prefix}'")
                pieces["error"] = f"tag '{full_tag}' doesn't match glob pattern '{tag_prefix}'"
                return pieces
            pieces["closest-tag"] = full_tag
        else:
            if not full_tag.startswith(tag_prefix):
